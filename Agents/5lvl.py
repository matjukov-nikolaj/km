...
url = 'https://mooped.net/local/its/tournament/agentaction/?gameid=GAMEID&userid=USERID&act=ACT'
url = 'https://mooped.net/local/its/tournament/agentaction/?id=2&userid=2&act=noAct noAct'
...
def connect(case_id, user_id, mapnum=1, passivAct = 'noAct', activAct = 'noAct'):
  ...
  resp = requests.get(url, params= {'tid': tid, 'userid': user_id, 'passive': passivAct, 'active': activAct}, verify=False) #proxies = proxyDict, verify=False)
...
def chooseAct(percept, allCaves, visitedCaves, act, windCaves, holeCaves) :
  ...
  if(curcave['isGold']) : # берем золото
    ...
    activAct = 'Take'
      ...
      if len(allCaves) - len(visitedCaves) == 1 or len(allCaves) - len(visitedCaves)  == 2:
          ...
          for i in range(len(dirlist)):
              nextCave = getNextCave(curcave['cNum'], dirlist[i])
              if nextCave in holeCaves:
                  newdirlist = list(dirs.keys())
                  newdirlist.remove(dirlist[i])
                  randdir = random.randint(0, len(newdirlist) - 1)
                  passivAct = chooseRotation(alldirsN[IAdir], alldirsN[newdirlist[randdir]])
          if curcave['cNum'] in windCaves:
  ...
  if(request == None) :
    print("Связь с сервером", url, "не установлена")
      ...
      while(request['error'] == None) :
        ...
        act = chooseAct(percept, allCaves, visitedCaves, lastAct, windCaves, holeCaves)
        lastAct = act
